...
def fuse_bn_into_conv(onnx_model):
    ...
    while current_node < len(onnx_model.graph.node):
		...
		#bn should has 5 inputs and all inputs should not be empty
            ...
            position = 0
            tmp_conv_w = np.ndarray(channel * (conv_w.shape[0] // channel))
            tmp_conv_b = np.ndarray(channel)
            for i in range(0, channel):
                s = bn_s[i] / math.sqrt(bn_v[i] + eps)
                b = bn_b[i] - bn_m[i] * s
                for j in range(0, conv_w.shape[0] // channel):
                    tmp_conv_w[position] = conv_w[position] * s
                    position += 1
                tmp_conv_b[i] = conv_b[i] * s + b

            onnx_model.graph.initializer[initializers_name_to_id_dict[conv_node.input[1]]
                                         ].raw_data = tmp_conv_w.tobytes()
            if len(conv_node.input) > 2:
                onnx_model.graph.initializer[initializers_name_to_id_dict[conv_node.input[2]]].raw_data = tmp_conv_b.tobytes(
                )
            else:
